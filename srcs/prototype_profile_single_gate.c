...
int main(void)
{
	...
	pthread_mutex_init(&counter_gate, NULL);
	...
	while(++i < PHILO_COUNT)
	{
		...
		philo[i].counter_gate = &counter_gate;
	...
	while (true)
	{
		pthread_mutex_lock(&counter_gate);
...
void	*routine(void *arg)
{
	...
	philo = (t_data *)(arg);
	pthread_mutex_lock(philo->counter_gate);
	++*philo->counter;
	pthread_mutex_unlock(philo->counter_gate);
